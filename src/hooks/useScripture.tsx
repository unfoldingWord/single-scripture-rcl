...
export function useScripture({ // hook for fetching scripture
  ...
  async function fetchBook(fetchParams, ignoreSha = null) {
    try {
      ...
      const shaToBeIgnored = ignoreSha && ignoreSha === resourceState?.sha

      if ((fetchLink === resourceState?.resource?.resourceLink) && !shaToBeIgnored) { // see if we already fetched this
        console.log(`useScripture.fetchBook() - Already fetching resourceLink is already ${resourceState?.resource?.resourceLink} and ignoreSha=${ignoreSha}`, fetchParams)
        return
      }
